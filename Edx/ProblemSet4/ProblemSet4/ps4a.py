...
def isValidWord(word, hand, wordList):
    ...
    copyhand = hand.copy()
    for c in word:
        if copyhand.has_key(c) and copyhand[c]>0:
            copyhand[c]-=1
        else:
            return False
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand) > 0:
        ...
        if word is '.':
            ...
            curr_score = getWordScore(word,length)
            score+=curr_score
            print '"{0}" earned {1} points. Total: {2} points'.format(word,curr_score,score)
...
if __name__ == '__main__':
    wordList = loadWords()
    compPlayHand({'h':1, 'i':1, 'c':1, 'z':1, 'm':2, 'a':1}, wordList, 7)
    playGame(wordList)
